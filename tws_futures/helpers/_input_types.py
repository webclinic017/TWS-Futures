...
class _Duration:
    ...
    def __call__(self, duration):
        try:
            assert isinstance(duration, int), f'Expected a positive integer less than ' \
                                              f'7, recieved: {duration}'
...
INPUT_TYPES = dict(date=_Date(), time=_Time(), duration=_Duration(), bar_size=_BarSize(),
                   file=_File(), url=_URL(), list=_TickersList())
